...
public class CreateOrderWithIntakeCommandTests
{
    ...
    public async Task Creates_Subject_Order_And_IntakeSession_Atomically()
    {
        ...
        CreateOrderWithIntakeResult? result;
        using (var scope = factory.Services.CreateScope())
        {
            var mediator = scope.ServiceProvider.GetRequiredService<IMediator>();

            var commandResult = await mediator.Send(new CreateOrderWithIntakeCommand(
    ...
    public async Task Reuses_Existing_Subject_By_Email()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var firstResult = await mediator.Send(new CreateOrderWithIntakeCommand(
    ...
    public async Task Does_Not_Duplicate_Phone_If_Already_Exists()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var secondResult = await mediator.Send(new CreateOrderWithIntakeCommand(
                "same-phone@example.com",
    ...
    public async Task Fails_When_Email_Is_Empty()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var result = await mediator.Send(new CreateOrderWithIntakeCommand(
                "",
                "+15551234567",
                customerId,
                "policy-v1")
