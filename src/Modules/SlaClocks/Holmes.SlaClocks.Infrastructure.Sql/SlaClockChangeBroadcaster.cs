...
public sealed class SlaClockChangeBroadcaster : ISlaClockChangeBroadcaster
{
    ...
    private readonly ConcurrentQueue<SlaClockChange> _history = new();

    private readonly ConcurrentDictionary<Guid, Subscription> _subscribers = new();

    public async Task PublishAsync(SlaClockChange change, CancellationToken cancellationToken)
    {
    ...
    public SlaClockChangeSubscription Subscribe(
        IReadOnlyCollection<UlidId>? orderFilter,
        UlidId? lastEventId,
        CancellationToken cancellationToken
    )
    {
        var channel = Channel.CreateUnbounded<SlaClockChange>();
        var id = Guid.NewGuid();
        ...
        return new SlaClockChangeSubscription(id, channel.Reader);
    }
    ...
    private sealed record Subscription(
        IReadOnlyCollection<UlidId>? OrderFilter,
        Channel<SlaClockChange> Channel
    )
    {
        ...
        public bool ShouldReceive(SlaClockChange change)
        {
