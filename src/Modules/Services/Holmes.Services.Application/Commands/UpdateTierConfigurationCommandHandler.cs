...
) : IRequestHandler<UpdateTierConfigurationCommand, Result>
{
    ...
    public async Task<Result> Handle(UpdateTierConfigurationCommand request, CancellationToken cancellationToken)
    {
        ...
        var tiers = config.Tiers.ToList();
        var tierIndex = tiers.FindIndex(t => t.Tier == request.Tier);
        ...
        tiers[tierIndex] = existing with
        {
            Name = request.Name ?? existing.Name,
            Description = request.Description ?? existing.Description,
            RequiredServices = request.RequiredServices ?? existing.RequiredServices,
            OptionalServices = request.OptionalServices ?? existing.OptionalServices,
            AutoDispatch = request.AutoDispatch ?? existing.AutoDispatch,
            WaitForPreviousTier = request.WaitForPreviousTier ?? existing.WaitForPreviousTier
        };

        var updatedConfig = new CatalogConfigDto(config.Services.ToList(), tiers);
